...
{: shortdesc}
  ...
  * 使用服務認證直接配置應用程式實例。如需詳細資料，請參閱[讓外部應用程式能使用 {{site.data.keyword.Bluemix_notm}} 服務](../manageapps/reqnsi.html#accser_external){: new_window}。
  * 建立使用者提供的服務作為橋接器。
	...
	若要使用存在於另一個地區的服務實例，請完成下列步驟：
      ...
      4. 使用您從 *VCAP_SERVICES* 環境變數記錄的認證及連線參數，建立使用者提供的服務實例。如需如何建立使用者提供之服務實例的相關資訊，請參閱[建立使用者提供的服務實例](../manageapps/reqnsi.html#user_provide_services)。
